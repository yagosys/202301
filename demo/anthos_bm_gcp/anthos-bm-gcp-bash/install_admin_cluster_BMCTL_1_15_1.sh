...
declare -a VMs=("$VM_WS" "$VM_ADMIN_CP")
#declare -a VMs=("$VM_WS")
...
for vm in "${VMs[@]}"
do
    while ! gcloud compute ssh ubuntu@"$vm" --zone "${ZONE}" --command "printf SSH to $vm succeeded"
    do
...
for vm in "${VMs[@]}"
do
    gcloud compute ssh ubuntu@"$vm" --zone "${ZONE}" << EOF
        sudo su
...
gcloud compute ssh ubuntu@$VM_WS --zone "${ZONE}" << EOF
sudo su
set -x
...
---
gcrKeyPath: /home/ubuntu//bm-gcr.json
sshPrivateKeyPath: /root/.ssh/id_rsa
gkeConnectAgentServiceAccountKeyPath: /home/ubuntu/bm-gcr.json
gkeConnectRegisterServiceAccountKeyPath: /home/ubuntu/bm-gcr.json
cloudOperationsServiceAccountKeyPath: /home/ubuntu/bm-gcr.json
---
apiVersion: v1
kind: Namespace
metadata:
