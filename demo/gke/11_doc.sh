...
command="nodeName=\$(kubectl get nodes -o jsonpath='{.items[*].metadata.name}') && podName=\$(kubectl get pods -l app=fos --field-selector spec.nodeName=\"\$nodeName\" -o jsonpath='{.items[*].metadata.name}') && kubectl exec -it po/\$podName -- iptables -L -t nat --verbose | grep MASQ "
