...
while true; do
  ...
  if [ "$previous_node_count" -ne "$node_count" ]; then
    ...
    #echo get cfos pod ip 
    #getCfosPodIp
    init_create_firewallpolicy
...
function createcfosfirewallpolicy {
      ...
      cfosurl=$ip
              ...
              then 
                echo $policyname $policyid with $SRC_ADDR_GROUP created on cfos $ip with url= $cfosurl
              fi
      done
...
function init_create_firewallpolicy {
